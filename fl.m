...
%      performs only the dicom-to-nifti conversion of subjet /scanner/dicom folder
%
...
%   Input config files:
%       preprocessing_[pipeline_id].cfg file in $FLDATA/[Experiment_code]/config detailing data preprocessing steps and options (see pipeline_info in HELP FL_PROCESS)
...
if ~ismember(lower(char(STEPS)),{'init','initforce','root','qa.plots','qaplots','secondlevel','secondlevel.roi'}); %,'list'}) % subject-expansion
...
switch(lower(STEPS))
    ...
    case {'preprocessing.report','preprocessing.report.gui','preprocessing.delete','parallel.report','parallel.report.gui','parallel.delete','report','report.gui','delete'}
        ...
        fl_process(regexprep(lower(STEPS),'^preprocessing\.','parallel.'),dataset);
    ...
    case {'preproc','preprocessing'}  % preproc ACE01 try01
        ...
        if ~isempty(dir(pipeline_info_opt)), 
            ...
            else pipeline_info=evlab17_parsefile(pipeline_info); 
            end
            pipeline_info=evlab17_parsefile(pipeline_info_opt,pipeline_info);
        end
        opts=varargin(3:end);
    ...
    case {'model','firstlevel','firstlevel.contrast'}  % firstlevel ACE01 try01 Speech_Raw
        ...
        if ~isempty(dir(analysis_info)), design_info=evlab17_parsefile(analysis_info,design_info); end
        if isanalysis_info_opt, 
            design_info=evlab17_parsefile(analysis_info_opt,design_info);
        elseif ~isempty(dir(subject_info)), % note: if no subject-specific (e.g. config/EXAMPLE01_speech.cfg) file is found, it will re-read the info in config/EXAMPLE01.cfg looking for design info there
            design_info_temp=evlab17_parsefile(subject_info,design_info);
    ...
    case {'firstlevel.plot','firstlevel.plots'}
        ...
        project_id=regexprep(subject_id,'[\d\*]+.*$','');
        dataset=fullfile(OUTPUT_FOLDER,project_id,'derivatives',pipeline_id,sprintf('sub-%s',subject_id));
        opts={'model.plots',dataset,design_id,...
            varargin{4:end}};
        fprintf('Project id %s\n',project_id);
        fprintf('Subject id %s\n',subject_id);
        fprintf('First-level analysis id %s\n',design_id);
    ...
    case {'secondlevel'}  % secondlevel ACE try01 Speech_Raw Group_Comparison
        ...
        results_cfg=evlab17_parsefile(results_info);
    ...
    case {'secondlevel.plot'}  % secondlevel ACE try01 Speech_Raw Group_Comparison
        ...
        options=varargin(5:end);
        results_path=fullfile(OUTPUT_FOLDER,project_id,'derivatives',pipeline_id,'results','secondlevel',design_id,results_id);
        fh=evlab17_resultsplots(results_path,options{:});
        varargout={fh};
    ...
    case {'secondlevel.roi.plot','secondlevel.rois.plot','secondlevel.wroi.plot','secondlevel.wrois.plot'}  % secondlevel.roi.plot ACE try01 Speech_Raw Group_Comparison SpeechROIs
        ...
        evlab17_roiresultsplots(rex_file);
    ...
    case 'root'
        pwd0=fl_process('utils_rootfolder',varargin{:});
        if isempty(varargin), 
            if ~nargout, disp(pwd0); 
            else varargout={pwd0};
            end
        end
        
    case {'init','initforce'}
        fl_process(STEPS,varargin{:});
    ...
    case {'fixpermissions','setpermissions'}
        ...
        if numel(varargin)>=2, 
            pipeline_id=varargin{2}; 
            dataset=fullfile(OUTPUT_FOLDER,project_id,'derivatives',pipeline_id,sprintf('sub-%s',subject_id));
            fl_process('utils_setpermissions',dataset,[],true);
